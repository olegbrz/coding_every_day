...
def execute(code: List[str]) -> Tuple[int, bool]:
    """execute iterates over the code and computes the accumulator variable,
    the execution  halts if one instruction is executed for the second time.

    Args:
        code (List[str]): input of the challenge converted to list.

    Returns:
        Tuple[int, bool]: the first number is the value of the accumulator when
        halted and the boolean indicates whether the execution was halted
        (False) or  terminated (True).
    """
    ins = [[line, False] for line in code]
    acc = 0
    i = 0
    while i < len(ins):
        if ins[i][1]:
            return acc, False
        opcode = ins[i][0].split()[0]
        value = ins[i][0].split()[1]
        ins[i][1] = True
...
def repair(code: List[str]) -> int:
    """repair iterates over the code changing every nop for jmp and vice versa,
    and then executing it. If the execution is successful, the funcition
    ...
    Returns:
        int: the value of the accumulator of the repaired code.
    """
    for i in range(len(code)):
        ins = code.copy()
        opcode = ins[i].split()[0]
        ...
        if execute(ins)[1]:
            return execute(ins)[0]


data = get_input()

print(f'Result: {execute(data)[0]}')
print(f'Result: {repair(data)}')
