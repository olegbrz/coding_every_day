...
def get_final_layout1(data):
    grid = np.copy(data)
    for i in range(1, len(grid)-1):
        for j in range(1, len(grid[0])-1):
            adj = [data[i-1, j-1], data[i-1, j], data[i-1, j+1],
                   data[i, j-1],                 data[i, j+1],
                   data[i+1, j-1], data[i+1, j], data[i+1, j+1]]
            if data[i, j] == 'L' and adj.count('#') == 0:
                grid[i, j] = '#'
            elif data[i, j] == '#' and adj.count('#') >= 4:
                grid[i, j] = 'L'
    if (data == grid).all():
        return grid
    else:
        return get_final_layout1(grid)


def get_final_layout2(data):
    grid = np.copy(data)
    for i in range(0, len(grid)):
        for j in range(0, len(grid[0])):
            ...
            diag_j = j - 1
            diag_i = i - 1
            while diag_j >= 0 and diag_i >= 0 and not lu:
                if data[diag_i, diag_j] in ['L', '#']:
                    lu = data[diag_i, diag_j]
                diag_j -= 1
                diag_i -= 1
            diag_j = j + 1
            diag_i = i - 1
            while diag_j < len(data[0]) and diag_i >= 0 and not ru:
                if data[diag_i, diag_j] in ['L', '#']:
                    ru = data[diag_i, diag_j]
                diag_j += 1
                diag_i -= 1
            diag_j = j - 1
            diag_i = i + 1
            while diag_j >= 0 and diag_i < len(data) and not ld:
                if data[diag_i, diag_j] in ['L', '#']:
                    ld = data[diag_i, diag_j]
                diag_j -= 1
                diag_i += 1
            diag_j = j + 1
            diag_i = i + 1
            while diag_j < len(data[0]) and diag_i < len(data) and not rd:
                if data[diag_i, diag_j] in ['L', '#']:
                    rd = data[diag_i, diag_j]
                diag_j += 1
                diag_i += 1

            left = np.delete(data[i, :j], np.where(data[i, :j] == '.'))
            right = np.delete(data[i, j+1:], np.where(data[i, j+1:] == '.'))
            up = np.delete(data[:i, j], np.where(data[:i, j] == '.'))
            down = np.delete(data[i+1:, j], np.where(data[i+1:, j] == '.'))
            ...
            if data[i, j] == 'L' and adj.count('#') == 0:
                grid[i, j] = '#'
            elif data[i, j] == '#' and adj.count('#') >= 5:
                grid[i, j] = 'L'

    if (data == grid).all():
        return grid
    else:
        return get_final_layout2(grid)


data_part1 = np.pad(np.array([[j for j in i] for i in get_input()]), (1, 1))
data_part2 = np.array([[j for j in i] for i in get_input()])


part1 = np.count_nonzero(get_final_layout1(data_part1) == '#')
part2 = np.count_nonzero(get_final_layout2(data_part2) == '#')
