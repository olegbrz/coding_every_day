...
def get_first_bus(data: List[str]) -> int:
    ...
    while not found:
        for id_ in bus_id:
            if i % id_ == 0:
                waiting_time = i-timestamp
                result = id_ * waiting_time
                found = True
        i += 1
...
def match_offsets(data: List[str]) -> int:
    ...
    remainder = cn_remainder(bus_id, offsets)
    product = reduce(lambda x, y: x*y, bus_id)

    return product - remainder


print(f'Result: {get_first_bus(data)}')
print(f'Result: {match_offsets(data)}')
