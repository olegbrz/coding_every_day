...
data = list(map(int, get_input()[0].split(',')))


def compute_last(start_set: List[int], number: int) -> int:
    """compute_last computes the nth number of the elves game given an input
    set.

    Args:
        start_set (List[int]): the input starting set parsed to ints.
        number (int): target number.

    Returns:
        int: the nth number of the game.
    """
    memory = {}
    for turn in range(len(start_set)):
        memory[start_set[turn]] = turn+1
    last = data[-1]
    for turn in range(len(start_set)+1, number+1):
        if last in memory.keys():
            turn_old = memory[last]
            memory[last] = turn-1
            last = turn-turn_old-1
        else:
            memory[last] = turn-1
            last = 0
    return last
...
print(f'Result {compute_last(data, 2020)}')
print(f'Result {compute_last(data, 30_000_000)}')
