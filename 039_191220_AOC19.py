...
def build_re(rules, code=0):
    for line in rules:
        if int(line[:line.find(':')]) == code:
            rule = line.split(': ')[1]
            if '"' in rule:
                return f'{rule[1]}'
            elif '|' in rule:
                ...
                if len(rule) == 2:
                    return f'({build_re(rules, rule[0])}|{build_re(rules, rule[1])})'
                else:
                    return f'({build_re(rules, rule[0])}{build_re(rules, rule[1])}|{build_re(rules, rule[2])}{build_re(rules, rule[3])})'
            else:
                rule = rule.split()
                rule = list(map(int, rule))
                if len(rule) == 1:
                    return f'{build_re(rules, rule[0])}'
                else:
                    return f'{build_re(rules, rule[0])}{build_re(rules, rule[1])}'


pattern = build_re(rules, 0)
pattern = re.compile(pattern)
counter = 0
for line in data[131:]:
    if bool(pattern.fullmatch(line)):
        counter += 1
