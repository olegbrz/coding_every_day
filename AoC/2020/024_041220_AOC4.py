...
def parse_passports(data):
    """parse_passports gets strings from input and parses them to a list of
 passport dictionaries.

    Args:
        data (List[str]): list with strings from input

    Returns:
        List[dict]: list with all passports in dict format (field:value)
    """
    passports = []
    temp_passport = []
    line = 0
    while line <= len(data):
        if line == len(data) or not data[line]:
            passport_to_dict = {}
            for key_value in ' '.join(temp_passport).split():
                k, v = key_value.split(':')
                passport_to_dict[k] = v
            passports.append(passport_to_dict)
            temp_passport = []
        else:
            temp_passport.append(data[line])
        line += 1
...
def is_valid(passport, strict=False):
    """is_valid checks if a passport dict is compliant with the rules

    Args:
        passport (dict): dictionary of the passport with all the fields
        strict (bool, optional): use strict double check or not. Defaults to
        False.

    Returns:
        bool: passport validity
    """
    # Regex to check if a HEX color value is valid
    pattern = re.compile("^#([a-fA-F0-9]{6}|[a-fA-F0-9]{3})$")

    # Check if the passport has all the fields
    for field in ['byr', 'iyr', 'eyr', 'hgt', 'hcl', 'ecl', 'pid']:
        if not (field in passport.keys()):
            return False

    if strict:
        # List of double check booleans
        checks = [
            ...
            bool(pattern.match(passport['hcl'])),
            # (Eye Color) - exactly one of: amb, blu, brn, gry, grn, hzl, oth.
            passport['ecl'] in ['amb', 'blu', 'brn', 'gry',
                                'grn', 'hzl', 'oth'],
...
def count_valid(passports, strict=False):
    """count_valid iterates over passwords and checks their validities in order
    to count how many are valid.

    Args:
        passports (List[dict]): list of dictionaries of the passports
        strict (bool, optional): use strict double check o rno. Defaults to
        False.
...
print(f'Result {count_valid(parse_passports(data), strict=False)}')
print(f'Result {count_valid(parse_passports(data), strict=True)}')
