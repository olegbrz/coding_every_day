...
def process_instructions(data: List[str]) -> int:
    """process_instructions processes the docking instructions with version 1.

    Args:
        data (List[str]): Input converted to list of strings.

    Returns:
        int: Sum of all values allocated in memory.
    """
    memory = {}
    mask = ''
    for line in data:
...
def floating_combinations(mask: str, address: int) -> List[int]:
    """floating_combinations computes all binary combination of a value with a
    floating mask.

    Args:
        mask (str): binary mask string.
        address (int): address in decimal format.

    Returns:
        List[int]: list of computed memory addresses.
    """
    x_count = mask.count('X')
    binary_combinations = list(product([0, 1], repeat=x_count))

    mask = list(mask)
    address = list(f'{address:#038b}'[2:])
    combinations = []
    for bit in binary_combinations:
        temp_addr = address.copy()
        floating_counter = 0
        for i in range(len(temp_addr)):
            if mask[i] == 'X':
                temp_addr[i] = str(bit[floating_counter])
                floating_counter += 1
            elif mask[i] == '1':
                temp_addr[i] = mask[i]
        combinations.append(int(''.join(temp_addr), 2))
    return combinations


def process_instructions2(data: List[str]) -> int:
    """process_instructions2 processes the docking instructions with version 2.
    ...
    for line in data:
        if 'mask' in line:
            ...
            address = int(line[line.find('[')+1: line.find(']')])
            value = int(line.split(' = ')[1])
            for combination in floating_combinations(mask, address):
                memory[combination] = value
    return sum(memory.values())


data = get_input()

print(f'Result {process_instructions(data)}')
print(f'Result {process_instructions2(data)}')
